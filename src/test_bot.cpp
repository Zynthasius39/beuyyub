...
void send_audio_raw(const dpp::slashcommand_t event, uint8_t* pcmdata, size_t &pcmsize) {
    dpp::voiceconn* v = event.from->get_voice(event.command.guild_id);
    v->voiceclient->send_audio_raw((uint16_t*)pcmdata, pcmsize);
}
...
int main() {
    ...
    size_t robot_size = 0;
    std::ifstream input ("test.pcm", std::ios::in|std::ios::binary|std::ios::ate);
    if (input.is_open()) {
